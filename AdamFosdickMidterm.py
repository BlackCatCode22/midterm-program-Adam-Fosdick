# dH 3/2/2023, revised 2023-03-10
# Mar2023Midterm.py
#
...
today = date.today()
print(today)
# Adam date.today() prints today date in real time
a_date = date(2019,3,15)
print(a_date)
# Adam how to create a custom date of your choice year , month , date
some_date = date(2000,1,2)
print(some_date)
# Adam you can name the variable anything you want when assigning to a date()
print("some_date = " + str(some_date))
# Adam combine a string and a variable by using +
a_date = today - some_date
print("some_date difference = " + str(a_date.days))
# Adam This overwrites the orignal value 03/15/2019 to today's date - 01/02/2000
# this will tell you only as a number not in years that will be calculated later
def calc_age_in_years(birth_date):
    today = date.today()
# Adam a function that takes the differance from the birth_date to today to find out how old they our
    ...
    return age_in_years
# Adam changed my_birthday to represent my real birthday
my_birthday = date(1986,1,15)
my_age_in_years = calc_age_in_years(my_birthday)
# calling calc_age_in_years function with the object of my_birthday witch we just defined
print("my_age_in_years is: " + str(my_age_in_years))
################ end of date object stuff #################################
# Adam printing space from this code to the next a new line for every \n
print("\n\n\n")
...
# Open the input file
# Adam created a txt file on my own computer and created a file path
my_file = open("C:/Users/mili_/cit-95/animalNames.txt", "r", encoding="utf-8")

# Read the file line by line into a data structure called 'Lines_names'
Lines_names = my_file.readlines()
# Adam know that we have all the data stored we can close the file
# Close the input file (if you open a file, be sure to close it)
my_file.close()

# Output each line in Lines
# Adam setting the line_count to 0
line_count = 0
# Adam this iterates each line one by one and creates a list of lines reading the file
...
# Adam list_of_lines[0] line 1 = Hyena Names: # list_of_lines[1] = empty sting
# Adam hyena_names_list is listed as line 3 but because we start counting at 0 it really [2]
hyena_names_list = list_of_lines[2].replace(',',"").split()
# Adam list_of_lines[3] = empty sting
# Adam list_of_lines[4] line 5 = Lion Names: # list_of_lines[5] = empty sting
# Adam Lion_names_list is listed as line 7 but because we start counting at 0 it really [6]
# Adam and so on just want you to know I understand what's happening and understand list I won't keep going
...
# Adam it was having an error reading the match switch case so i turned it into an if-else statment
# Adam not sure if this has anything to do with it not writing the file i dont belive so
...
# Adam we are initialize our global variables and list
# Adam global variables can be acceded at any point during the program
# Adam instead of having some inside the scope of a function you cant access them outside that scope
num_of_hyenas = 0
num_of_lions = 0
num_of_tigers = 0
num_of_bears = 0
...
# Adam created a txt file on my own computer and created a file path
my_file = open("../cit-95/arrivingAnimals.txt", "r", encoding="utf-8")
...
for element in my_array:
    ...
    my_file = open("C:/Users/mili_/cit-95/adamOutput.txt", "w", encoding="utf-8")

    my_file.write("Midterm Program Output; by Adam Fosdick, Spring 2023, Fresno, CA\n\n")
